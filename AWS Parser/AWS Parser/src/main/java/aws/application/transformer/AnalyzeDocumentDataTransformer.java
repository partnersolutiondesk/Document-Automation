...
public class AnalyzeDocumentDataTransformer {
    ...
    public EngineData transform(
            AnalyzeDocumentResponse analyzeDocument, String inputImageFilePath, Domain domain) {
        ...
        EngineData engineData =
                EngineData.newBuilder()
                        ...
                        .setExtractionResult(
                                toExtractionResult(analyzeDocument, imagePreprocessingResult))
                        .build();
        return engineData;
    }
    ...
    private ImagePreprocessingResult toImagePreprocessingResult(
            AnalyzeDocumentResponse analyzeDocument, String inputImageFilePath) {
    ...
    private ExtractionResult toExtractionResult(
        ...
        toKeyValuePairs(blocksByTypeMap, extractionResultBuilder, imagePreprocessingResult);
        toTables(blocksByTypeMap, extractionResultBuilder, imagePreprocessingResult);
    ...
    private void toTables(
        ...
        Map<Integer, Block> headerRow = rowMap.remove(1);
        Map<Integer, String> headerMap = new HashMap<>();

        for (Map.Entry<Integer, Block> headerEntry : headerRow.entrySet()) {
            ...
            headerMap.put(headerEntry.getKey(), keyText);
        ...
        for (Map.Entry<Integer, Map<Integer, Block>> rowEntry : rowMap.entrySet()) {
            ...
            for (Map.Entry<Integer, Block> cellEntry : cellMap.entrySet()) {
                ...
                String headerText = headerMap.get(cellEntry.getKey());
                int pageNumber = getPageNumber(block);
                TableCellFeature.Builder tableCellFeatureBuilder =
                        TableCellFeature.newBuilder()
                                .setId(block.id())
                                .setDomainFieldKey(headerText)
                                .setText(valueText)
                                .setExtractedDataType(DataType.TEXT)
                                .setGeometry(toGeometry(block, imagePreprocessingResult))
                                .setPageNum(pageNumber);
    ...
    private DocDetectResult toDocDetectResult(AnalyzeDocumentResponse analyzeDocument) {
